...
int main() {
    ...
    REPEAT(l, numTestCases) {
        ...
        REPEAT(i, numMolecules) {
            REPEAT(j, numMolecules) if (molecules[i][0] != molecules[j][0]) ratios[numRatios++] = (molecules[j][1]-molecules[i][1])/(molecules[i][0]-molecules[j][0]);
        }
        ratios[numRatios++] = 1e-10;
        ratios[numRatios++] = 1e10;
        
        REPEAT(k, numRatios) if (ratios[k] > 1e-11) {
            curRatio = ratios[k]-1e-11;
            ...
            addElementByKey(&allOrders, curOrder, 1, true);
        }
