...
void process(uint64_t length) {
	...
	for(i=0; i < length-1;i++) {
		gcd_result =  gcd_nr(input_number[i],input_number[i+1]);
        	decrypted_numbers[i] = input_number[i]/gcd_result;
		sorted_numbers[i] = decrypted_numbers[i];
	}
       	decrypted_numbers[i] = input_number[i-1]/ decrypted_numbers[i-1];
	sorted_numbers[i] = decrypted_numbers[i];
	i++;
       	decrypted_numbers[i] = input_number[i-1]/ decrypted_numbers[i-1];
	sorted_numbers[i] = decrypted_numbers[i];
