...
int main(void){
	...
	while(t--){
		int N,L,i,j;
		scanf("%d %d",&N,&L);
		long long int prime[L],fact1=0,fact2=0,fact3=0,decrypt[L+1],sort[L+1],alpha[26];
		char str[L+2];
		for(i=0;i<L;i++){
			scanf("%lld",&prime[i]);
		}
		...
		for(i=0;i<=L;i++){
			int x = binarySearch(alpha, 0, L, decrypt[i]); 
			str[i] = 65+x;
		}
		str[i]='\0';
		//char first[32] = "CJQUIZKNOWBEVYOFDPFLUXALGORITHMS";
		//first[32]='\0';
		printf("Case #%d: %s",count,str);
		//printf("%d",strcmp(first,str));
		count++;
	}
	return 0;
}