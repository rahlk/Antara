		...
		for (i = 1; i < L; i++) {
			...
			for (j = 0; j < idx; j++) {
				...
				if (arr[i] % factor[j] == 0 && arr[i + 1] % factor[j] == 0) {
					...
					if (arr[i + 1] % (arr[i] / factor[j]) == 0 && check(arr[i + 1] / (arr[i] / factor[j]), idx))
						factor[idx++] = arr[i + 1] / (arr[i] / factor[j]);
					...
					if (arr[i] % (arr[i + 1] / factor[j]) == 0 && check(arr[i] / (arr[i + 1] / factor[j]), idx))
						factor[idx++] = arr[i] / (arr[i + 1] / factor[j]);
