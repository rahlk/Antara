    ...
    for (int cas = 1; cas <= cases; cas++)
    {
        ...
        for (int i = 1; i <= N; i++)
        {
            for (int j = i + 1; j <= N; j++)
            {
                ...
                if (tmp < 0)
                {
                    array[pair].y = abs(input[i][0] - input[j][0]);
                    array[pair].x = abs(input[i][1] - input[j][1]);
                    pair++;
                }
            }
        }
            ...
            for (int i = 1; i < pair; i++)
            {
                ...
                if (c1 != c2)
                {
                    ...
                    if (yes == 1)
                    {
                       ...
                       /* if (ansC < ansJ)
                        {
                            for (int tc = 1; tc < ansC * 2; tc++)
                            {
                                long long int tmp = tc;
                                tmp *= array[i].y;
                                long long int mJ = tmp / array[i].x + 1;
                                
                                long long int p1 = array[i - 1].x;
                                p1 *= mJ;
                                long long int p2 = array[i - 1].y;
                                p2 *= tc;
                                if (p1 < p2)
                                {
