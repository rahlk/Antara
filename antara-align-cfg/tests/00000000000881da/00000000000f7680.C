...
int main(void) {
	...
	for (int asdf = 0; (unsigned)asdf < (unsigned)count; asdf++) {
		scanf("%d", &size);
		...
		if (size < 2) {
			...
			for (int i = 0; (unsigned)i < (unsigned)sizeof(out)/sizeof(char); i++) {
				out[i] = 0;
			}
			...
			for (int i = 0; (unsigned)i < (unsigned)sizeof(par)/sizeof(char); i++) {
				if (par[i] == 'E') {
					b++;
					arr[a][b] = 1;
				} else if (par[i] == 'S') {
