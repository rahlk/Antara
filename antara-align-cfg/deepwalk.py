...
class Deepwalk:
        ...
        with open('.deepwalk.embedding', 'r') as embedding_file:
            for i, line in enumerate(embedding_file):
                if i == 0:  # Ignore header
                    ...
                    contents = line.split(" ")
                    index.append(int(contents[0]))
                    embeddings.append(list(map(float, contents[1:])))
        
        # Preserve ordering
        ordering = np.argsort(index)
        embeddings = [embeddings[i] for i in ordering]
        return embeddings
    ...
    def get_node_embeddings(self):
        ...
        cmd += "--number-walks 64 --format edgelist --input .deepwalk.edgelist --output .deepwalk.embedding --undirected False"
        cmd = shlex.split(cmd)
        # Run command
