...
int compareRatios(const void *ptr1, const void *ptr2) {
    weight *ratio1 = (weight*)ptr1, *ratio2 = (weight*)ptr2;
    //r1[0]/r1[1] < r2[0]/r2[1] -> r1[0]*r2[1]-r1[1]*r2[0] < 0
    weight key = ratio1[0]*ratio2[1]-ratio1[1]*ratio2[0];
    if (key == 0) return 0;
    return (key > 0) ? 1 : -1;
}
...
int compareMolecules(const void *ptr1, const void *ptr2) {
    ...
    weight key1 = mol1[0]*curRatio[0]+mol1[1]*curRatio[1];
    weight key2 = mol2[0]*curRatio[0]+mol2[1]*curRatio[1];
...
int main() {
    ...
    REPEAT(l, numTestCases) {
        ...
        qsort(ratios, numRatios, 2*sizeof(weight), compareRatios);
