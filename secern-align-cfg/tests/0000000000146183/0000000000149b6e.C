...
int ok(int n,int *p,int *a,int *b) {
	...
	for(int w1=1;w1<=2000000000;w1+=4000000) for(int w2=1;w2<=2000000000;w2+=4000000) {
		for(int i=0;i<n-1;i++) {
			int j=i+1;
			int a1=a[p[i]];
			int a2=a[p[j]];
			int b1=b[p[i]];
			int b2=b[p[j]];
			if((long long)a1*w1+(long long)b1*w2 >= (long long)a2*w1+(long long)b2*w2) goto fail;
		}
		return 1;
	fail:;
	}
