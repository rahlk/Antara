    ...
    for (int cas = 1; cas <= cases; cas++)
    {
        ...
        for (int i = 1; i <= N; i++)
        {
            for (int j = i + 1; j <= N; j++)
            {
                ...
                if (tmp < 0)
                {
                    int ty = abs(input[i][0] - input[j][0]);
                    int tx = abs(input[i][1] - input[j][1]);
                    array[pair].y = ty / gcd(tx, ty);
                    array[pair].x = tx / gcd(tx, ty);
                    pair++;
                }
            }
        }
            ...
            for (int i = 1; i < pair; i++)
            {
                ...
                if (c1 != c2)
                {
                    ...
                    if (yes == 1)
                    {
                        ...
                        {
                            long long int tmp = tc;
                            tmp *= array[i - 1].y;
                            long long int mJ = tmp / array[i - 1].x + 1;
                                
                            long long int p1 = array[i].x;
                            p1 *= mJ;
                            long long int p2 = array[i].y;
                            p2 *= tc;
                            if (p1 < p2)
                            {
                                if (tc < ansC || (tc == ansC && mJ < ansJ))
                                {
                                    ansC = tc;
                                    ansJ = mJ;
                                }
