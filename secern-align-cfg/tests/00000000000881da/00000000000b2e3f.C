...
void print_path(struct MAZE_PATH *maze_path, uint32_t num_path)
{
    uint32_t i ;
    ...
    {
        debug_print(DBG_1, "(%u,%u,", maze_path[i].x, maze_path[i].y);
...
#if (DEBUG_LEVEL >= DBG_1)
void print_maze(uint32_t maze_size, struct MAZE_PATH *maze_path, uint32_t num_path)
{
...
#if 0
    print_maze(test_case.maze_size, test_case.new_path, test_case.path_index + 1);
#endif        
    if(0 < dir)
    {
        debug_print(DBG_2, "%s-%u %u (%u,%u,%s) %s\r\n", func_name, line, test_case.path_index, x, y, get_dir_string(dir), msg);
    }
    else
    {
        debug_print(DBG_2, "%s-%u %u (%u,%u) %s\r\n", func_name, line, test_case.path_index, x, y, msg);
    }
#endif    

#if (DEBUG_LEVEL >= DBG_1)
    print_path(test_case.new_path, test_case.path_index);
#endif    
}
...
#if (DEBUG_LEVEL >= DBG_1)
    print_maze(test_case.maze_size, test_case.old_path, num_path);
#endif    

#if (DEBUG_LEVEL >= DBG_1)
    print_path(test_case.old_path, num_path);
#endif    
    debug_print(DBG_1, "\n");
}
 ...
 *  
 */ 
void find_maze_path(uint32_t x, uint32_t y)
{
    int try_ok ;
    uint32_t new_x ;
    uint32_t new_y ;
    ...
    cur_index = test_case.path_index ;
    test_case.path_index++ ;
    ...
    if((MAX_MAZE_SIZE - 1) <= x) 
    {
        if((MAX_MAZE_SIZE - 1) <= y) 
        {
            ...
            test_case.new_path[cur_index].x = x ;
            test_case.new_path[cur_index].y = y ;
            return ;
        }
    }
    ...
    for(i = 0 ; i < 2 ; i++)
    {
        if(0 == test_case.path_found)
        {
            ...
            if(test_case.path_index < MAX_MAZE_PATH)
            {
                test_case.new_path[cur_index].x   = x ;
                test_case.new_path[cur_index].y   = y ;
                test_case.new_path[cur_index].dir = dir ;
            }
            ...
            if(test_case.maze_size <= new_x) 
            {
                try_ok = 0 ;
            }

            if(test_case.maze_size <= new_y) 
            {
                try_ok = 0 ;
            }
...
#if (DEBUG_LEVEL >= DBG_1)
    num_path = (2 * test_case.maze_size) - 2 ;
    print_maze(test_case.maze_size, test_case.new_path, num_path);
    compare_path();
#endif    
