...
#include<stdlib.h>
#include<stdbool.h>
bool solveMazeUtil(int** maze, int x, int y, int count,int N,char* sol); 
...
{ 
    ...
    {    count++;
        ...
        if (solveMazeUtil(maze, x+1, y,count, N sol) == true)
        { sol[count] = 'E'; return true;}
        
        if (solveMazeUtil(maze, x, y+1,count,N,sol) == true) 
