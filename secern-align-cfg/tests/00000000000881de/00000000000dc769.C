...
int T,N,B,F,sec,ret,nouse,use,list,recsec;
char buffer[1028];
int status[2][1024];
int result[15];
int skip[2][1024];
...
void divide(){
  ...
  for(int i = 0;i<tot;i++){
    ...
    //fprintf(stderr,"*** :  %d %d %c %c %d %d %d %d %d\n",ptr,N-B,buffer[ptr],cur,cnt,usesec,cnt+limiter,limiter,sec*2);
      cnt++;
      ptr++;
    }
    //fprintf(stderr,"*** :  %d %d %c %c %d %d %d %d %d\n",ptr,N-B,buffer[ptr],cur,cnt,usesec,cnt+limiter,limiter,sec*2);
...
void solve(){
  ...
  for(int i = 0;i<T;i++){
    ...
    sec = 0;
    for(int j = 0;j<B;j++){
      sprintf(&buffer[sec],"%d ",result[j]);
      sec += strlen(&buffer[sec]);
    }
    puts(buffer);
    fflush(stdout);
    scanf("%d",&ret);
