...
void traducir_cadena(char *mensaje, int *lista_traduccion, int *mensaje_cifrado, int l, int n_primos) {
	int i;
	for (i = 0; i <= l; i++) {
		mensaje[i] = 65 + blin(lista_traduccion, 0, n_primos-1, mensaje_cifrado[i]);
	}
	mensaje[i] = 0;
}
int main() {
	...
	for(int j = 0; j < t; j++) {
		...
		traducir_cadena(mensaje, lista_primos, mensaje_cifrado, l, n_primos);
		printf("Case #%d: %s\n", j+1, mensaje);
		free(producto);
