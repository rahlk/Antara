...
int main()
{
    ...
    for(j=0;j<=T-1;j++)
    {
    printf("give the limiting integer and length of the list");
    scanf("%d ",&N);
    scanf("%d",&L);
    printf("give the list of values from ciphertext");
    for(i=0;i<=L-1;i++)
    {
    scanf("%d",&arr[i]);
    }
   ...
   char Decrypt(int N, int L,char arr[L])
  {
      int i,j,i1,j1,i2,j2,i3,j3=0,prime[L+1],primetext[L+1],k=0,k1=0,result[L+1];
       for(i=2;i<=N;i++)
    {
        int c=0;
        for(j=1;j<=i;j++)
