...
/* This function takes last element as pivot, places 
   ...
   of pivot */
long long int partition (long long int arr[], long long int low, long long int high, long long int pos[]) 
{ 
...
/* The main function that implements QuickSort 
 arr[] --> Array to be sorted, 
  low  --> Starting index, 
  high  --> Ending index */
void quickSort(long long int arr[], long long int low, long long int high, long long int pos[]) 
{ 
    if (low < high) 
    { 
        /* pi is partitioning index, arr[p] is now 
           at right place */
        long long int pi = partition(arr, low, high, pos); 
...
//		}
		...
		for ( i = 1; i <= L; ++i )
		{
			if ( beta[i] != beta[i-1] )
			{
				...
				mensaje[pos[i]] =(char) jt+65;
				++jt;
			}
			else
			{
				mensaje[pos[i]] = mensaje[pos[i-1]];
			}
		}
//		printf("\n");
		printf("Case #%lld: ", k);
