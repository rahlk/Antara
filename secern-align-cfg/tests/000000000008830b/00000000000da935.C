...
void main()
{
	...
	while(counter > 0)
	{
		...
		char* inputarr2 = (char*)calloc(l+2,sizeof(char));
		...
		{
			for(int m = 0 ; m < l+1-k-1 ; m++)
			{
				if(alphabets[m] > alphabets[m+1])
				{
					int temp = alphabets[m];
					alphabets[m] = alphabets[m+1];
					alphabets[m+1] = temp;
				}
		...
		char* krash = strcpy(ans[index1].str,inputarr2);
		index1++;
		counter--;
	}
