...
int main()
{
	...
	for (tc=1; tc<=TC; tc++) {
		...
		memset(plaintext_int, 0, sizeof(plaintext_int));
		memset(cypertext, 0, sizeof(cypertext));
		...
		for (i=1; i<L; i++) {
			remainder1 = cypertext[i] % component[i-1];
			if (remainder1 == 0) {
		...
		plaintext_int[0] = cypertext[0] / component[1];
