#include<stdio.h>
unsigned long checkprimenumber(unsigned long n);
unsigned long sort(unsigned long array[], unsigned long n);
unsigned long removeduplicates(unsigned long arr[], unsigned long n);
unsigned long main()
{
 unsigned long f,y,T,k,n,i,flag,c=0,p[10000],l,v[10000],m[10000],j,temp1,temp2,s[10000],first,last,middle;
 unsigned char t[10000],r[1000][1000];
 scanf("%d",&T);
 for(k=1;k<=T;k++)
 {
  scanf("%d %d",&n,&l);
...
unsigned long removeduplicates(unsigned long arr[], unsigned long n)
{
 unsigned long temp[10000],i,j=0;
 for(i=0;i<n-1;i++)
 {
  if(arr[i]!=arr[i+1])
  {
