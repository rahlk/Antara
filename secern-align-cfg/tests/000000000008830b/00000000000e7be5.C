#include <stdio.h>
#include <stdlib.h>
#include <math.h>

int isPrime(long long int a)
{
    for(long long int i = 2 ; i <= sqrt(a) ; i+=1)
    {
        if(a%i == 0)
            return 0;
    }
    return 1;
}

void findDivisors(long long int a , long long int *arr , int m)
{
    if(m==0)
    {
        for(long long int i = 3 ; i <= sqrt(a) ; i+=2)
        {
            if(isPrime(i) && a%i == 0 && isPrime(a/i))
            {
                arr[0] = i;
                arr[1] = a/i;
                break;
            }
        }
    }
    else
    {
        arr[m+1] = a/arr[m];
    }
}

void sortArray(long long int arr[] , int l)
{
    for(int i = 0 ; i <= l ; i++)
    {
        for(int j = i+1 ; j <= l ; j++)
        {
            if (arr[i] > arr[j]) 
            {
                long long int a =  arr[i];
                arr[i] = arr[j];
                arr[j] = a;
            }
        }
    }
}

int main()
{
    int N;
    scanf("%d" , &N);
    for(int i = 1; i <= N ; i++)
    {
        int n , l;
        scanf("%d %d" , &n , &l);
        long long int arr[l];
        for(int j = 0 ; j < l ; j++)
        {
            scanf("%lld" , &arr[j]);
        }
        long long int divisors[l+1];
        for(int j = 0 ; j <= l ; j++)
        {
            findDivisors(arr[j] , divisors , j);
        }
        long long int sorted[l+1];
        for(int g = 0 ; g <= l ; g++)
        {
            sorted[g] = divisors[g];   
        }
        sortArray(sorted , l);
        long long int fin[26];
        int k = 0;
        for(int t = 0 ; t<=l ; t++)
        {
            if(t==0)
            {
                fin[k] = sorted[t];
                k++;
            }
            else
            {
                if(sorted[t] != sorted[t-1])
                {
                    fin[k] = sorted[t];
                    k++;
                }
            }
                
        }
        printf("Case #%d: " , i);
        for(int j = 0 ; j <= l ; j++)
        {
            for(int k = 0 ; k <= 25 ; k++)
            {
                if(fin[k] == divisors[j])
                {
                    printf("%c" , 65+k);
                    break;
                }
            }
        }
        if(i != N)
            printf("\n");
    }
    return 0;
}