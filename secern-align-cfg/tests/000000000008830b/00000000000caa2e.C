...
int main(){
  long t, n, l, x, a, b[4], i, newL, value, index, *inputList, *charList, *charList2;
  ...
  for (x = 0; x < t; x++) {
    ...
    b[0] = primeFactors(inputList[0]);
    b[1] = inputList[0]/b[0];
    b[2] = primeFactors(inputList[1]);
    b[3] = inputList[1]/b[2];
    a = (b[0]==b[2])?b[0]:(b[0]==b[3])?b[0]:b[1];
//    a = hcf(inputList[0],inputList[1]);
    charList = (long *)malloc((l + 5) * sizeof(long));
    charList2 = (long *)malloc((l + 5) * sizeof(long));
