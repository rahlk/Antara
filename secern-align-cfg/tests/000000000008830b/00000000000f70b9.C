...
int main(void)
{
	...
	for (testcase = 0; testcase < T; testcase++)
	{
		...
		{
			sameValueIdx = i;
			if (pangrams[i] != pangrams[i + 1])
				break;
		}
		a = getSubmultiple(pangrams[sameValueIdx]);
		b = pangrams[sameValueIdx] / a;

		if (pangrams[sameValueIdx+1] % a == 0)
		{
			subPangrams[sameValueIdx] = b;
			subPangrams[sameValueIdx+1] = a;
		}
		else
		{
			subPangrams[sameValueIdx] = a;
			subPangrams[sameValueIdx+1] = b;
		}
		for (i = sameValueIdx - 1; i >= 0; i--)
		{
			subPangrams[i] = pangrams[i] / subPangrams[i + 1];
		}

		for (i = sameValueIdx + 2; i <= L; i++)
		{
