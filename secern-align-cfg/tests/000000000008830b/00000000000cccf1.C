...
int main(){
  long t, n, l, x, a, b, i, newL, value, index, *inputList, *charList, *charList2;
  ...
  for (x = 0; x < t; x++) {
    ...
    for (i = 0; i < l-1; i++) {
      if(inputList[i] != inputList[i + 1]) {
        a = hcf(inputList[i],inputList[i+1]);
        b = i;
        break;
      }
    }
    ...
    charList[b] = inputList[0] / a;
    charList[b+1] = a;
    charList2[b] = charList[b];
    charList2[b+1] = charList[b+1];
    for (i = b-1; i >= 0; i--) {
