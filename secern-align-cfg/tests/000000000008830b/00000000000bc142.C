	...
	for(t=0;t<num_tests;t++)
		...
		for(i=1;i<l;i++)
			...
			if(input[i] % primes[j]==0)
				{
				primes[++p]=input[i]/primes[j];
				}
			if(input[i] % primes[j-1]==0)
				{
				primes[++p]=input[i]/primes[j-1];
				}
			j=p;
			}
		p++;
		memset(key,0,sizeof(key[0])*26);
		qsort(primes,p,sizeof(primes[0]),sort_primes);
		key[0]=primes[0];
		for(i=1,j=0;i<p;i++)
			{
			if(primes[i]!=key[j])
		...
		for(i=1;i<l;i++)
			...
			if(input[i] % key[output[i]-'A']==0)
				...
				for(p=i-1;p>0;p--)
					...
					for(j=0;(j<26)&&(key[j]);j++)
						{
						if(key[output[p]-'A'] * key[j]==input[p-1])
							{
							output[p-1]='A'+j;
							break;
							}
