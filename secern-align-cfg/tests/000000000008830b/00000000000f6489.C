    ...
    for (j = 2; j < MAX_DB_SIZE; j++) {                          
      ...
      if (pdb[j].is_prime == true) {                             
        if((ip[i].c % j)==0) {                                   
          ...
          //there may be more than one pair of prime multiple for a input.
          if ((i == 0) && ((ip[i+1].c%j == 0) || (ip[i+1].c%j_prime == 0))) { // special case for last element in input
            ip[i].c1 = j;                                          
            ip[i].c2 = j_prime;
            pdb[j].in_use = true;
            pdb[j_prime].in_use = true;
            break;
          } else if ((i == ip_size-1) && ((ip[i-1].c%j == 0) || (ip[i-1].c%j_prime == 0))) { // special case for last element in input
            ip[i].c1 = j;                                          
            ip[i].c2 = j_prime;
            pdb[j].in_use = true;
            pdb[j_prime].in_use = true;
            break;
          } else if ( (i>0) && (i < (ip_size-1)) && 
                      (((ip[i-1].c%j == 0) && (ip[i+1].c%j_prime == 0)) || 
                       ((ip[i+1].c%j == 0) && (ip[i-1].c%j_prime == 0))) ) {
            ip[i].c1 = j;                                          
            ip[i].c2 = j_prime;
            pdb[j].in_use = true;
